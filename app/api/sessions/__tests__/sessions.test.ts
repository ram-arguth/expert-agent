...
describe("Sessions API", () => {
  ...
  describe("GET /api/sessions", () => {
    ...
    it("respects limit parameter", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/sessions?limit=1",
      );
    ...
    it("truncates long message previews", async () => {
      ...
      expect(response.status).toBe(200);
      const body = await response.json();
      expect(body.sessions[0].lastMessage.preview.length).toBeLessThanOrEqual(
        153,
      ); // 150 + '...'
    });
  ...
  describe("GET /api/sessions/[sessionId]", () => {
    ...
    it("includes structured data in message response", async () => {
      ...
      const body = await response.json();
      expect(body.messages[1].structuredData).toEqual({
        summary: "Good design",
      });
    });

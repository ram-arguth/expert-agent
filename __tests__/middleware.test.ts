...
describe('Next.js Middleware', () => {
  ...
  describe('Trace Context', () => {
    it('adds trace context headers when not present', async () => {
      ...
      expect(response.headers.get('x-trace-id')).toBe('test-trace-id-12345678901234567890123456789012');
      expect(response.headers.get('x-span-id')).toBe('test-span-id-1234567890123456');
    });
  ...
  describe('E2E Test Principal Injection', () => {
    it('injects test session when E2E mode is allowed and valid', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/dashboard', {
        'X-E2E-Test-Principal': JSON.stringify({ id: 'test', email: 'test@example.com', name: 'Test' }),
  ...
  describe('Production Guard', () => {
    it('blocks requests with test headers in production', async () => {
      const forbiddenResponse = new Response(
        JSON.stringify({ error: 'Forbidden', message: 'Test mode not available' }),
        { status: 403 }
      );

      vi.mocked(createProductionGuardMiddleware).mockReturnValue(() => forbiddenResponse);

...
describe("Metrics Module", () => {
  ...
  describe("Counter Metrics", () => {
    it("increments counter by 1 by default", () => {
      incrementCounter("request.count", 1, {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
      const value = getCounterValue("request.count", {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
    ...
    it("increments counter by specified value", () => {
      incrementCounter("ai.tokens.input", 100, {
        agent_id: "ux-analyst",
        model: "gemini-3",
      });
      incrementCounter("ai.tokens.input", 50, {
        agent_id: "ux-analyst",
        model: "gemini-3",
      });
      const value = getCounterValue("ai.tokens.input", {
        agent_id: "ux-analyst",
        model: "gemini-3",
      });
  ...
  describe("Histogram Metrics", () => {
    it("records histogram values", () => {
      recordHistogram("request.latency", 50, {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
      recordHistogram("request.latency", 100, {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
      recordHistogram("request.latency", 150, {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });

      const stats = getHistogramStats("request.latency", {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
    ...
    it("tracks histograms with different labels separately", () => {
      ...
      const statsA = getHistogramStats("request.latency", {
        endpoint: "/api/a",
        method: "GET",
        status_code: "200",
      });
      const statsB = getHistogramStats("request.latency", {
        endpoint: "/api/b",
        method: "GET",
        status_code: "200",
      });
  ...
  describe("Duration Measurement", () => {
    it("measures async operation duration", async () => {
      ...
      // Timer resolution can vary; accept >= 8ms for 10ms setTimeout
    ...
    it("records failure for async operation that throws", async () => {
      ...
      const successStats = getHistogramStats("ai.latency", {
        agent_id: "test",
        model: "gemini-3",
        success: "true",
      });
      const failureStats = getHistogramStats("ai.latency", {
        agent_id: "test",
        model: "gemini-3",
        success: "false",
      });
    ...
    it("measures sync operation duration", () => {
      const result = measureDurationSync(
        "query.duration",
        () => {
          // Simulate some work
          let sum = 0;
      ...
      const stats = getHistogramStats("query.duration", {
        agent_id: "test",
        success: "true",
      });
      expect(stats).toBeDefined();
  ...
  describe("High-Level Metric Helpers", () => {
    ...
    describe("recordTokenUsage", () => {
      it("records input and output tokens", () => {
        ...
        expect(
          getCounterValue("ai.tokens.input", {
            agent_id: "ux-analyst",
            model: "gemini-3-pro",
          }),
        ).toBe(1000);
        expect(
          getCounterValue("ai.tokens.output", {
            agent_id: "ux-analyst",
            model: "gemini-3-pro",
          }),
        ).toBe(500);
    ...
    describe("Security Metrics", () => {
      it("records security blocks", () => {
        ...
        expect(
          getCounterValue("security.blocked", {
      ...
      it("records PII detection", () => {
        ...
        expect(
          getCounterValue("security.pii_detected", {
            pii_type: "ssn",
            direction: "input",
            action: "block",
          }),
    ...
    describe("Rate Limiting Metrics", () => {
      it("records rate limit exceeded", () => {
        ...
        expect(
          getCounterValue("rate_limit.exceeded", {
            tier: "free",
    ...
    describe("Cost Metrics", () => {
      it("records estimated cost", () => {
        ...
        expect(
          getCounterValue("cost.estimated", {
            user_id: "user-1",
            org_id: "org-1",
          }),
        ).toBe(150);
    ...
    describe("Session Metrics", () => {
      it("records session creation", () => {
        ...
        expect(
          getCounterValue("session.created", {
            org_id: "org-1",
            agent_id: "ux-analyst",
          }),
        ).toBe(2);
  ...
  describe("clearMetrics", () => {
    it("clears all metrics", () => {
      incrementCounter("request.count", 10, {
        endpoint: "/api/test",
        method: "GET",
        status_code: "200",
      });
  ...
  describe("Invalid Metric Handling", () => {
    it("warns on invalid counter metric", () => {
      ...
      incrementCounter(
        "invalid.metric" as keyof typeof METRIC_DEFINITIONS,
        1,
        {},
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Unknown or invalid"),
      );
